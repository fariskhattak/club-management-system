...
import BudgetBarChart from "@/components/BudgetBarChart"; // Import the BudgetBarChart component
...
const FinancesView: React.FC<FinancesViewProps> = ({ currentClub }) => {
    ...
    const fetchSponsors = async () => {
        ...
        try {
            const response = await fetch(
                `http://localhost:5001/api/clubs/${currentClub.club_id}/sponsors`
            );
            if (response.ok) {
    ...
    return (
        <div className="p-4 bg-white rounded shadow text-black">
            ...
            {currentClub && (
                <div className="mb-12">
                    <BudgetBarChart clubId={currentClub.club_id} />
                </div>
            )}
