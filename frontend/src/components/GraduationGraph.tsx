...
const GraduationGraph: React.FC<PieGraphProps> = ({ currentClub, refreshGraphTrigger }) => {
    ...
    const fetchGraduationYears = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Group members by graduation year and count occurrences
          const graduationCounts = members.reduce((acc: Record<string, number>, member: Member) => {
          ...
          const graduationData = Object.entries(graduationCounts).map(([year, count]) => ({
            name: year,
  ...
  if (!currentClub) {
    return (
      <div className="w-full h-96 p-4 bg-white rounded-lg shadow-md flex items-center justify-center">
        <p className="text-gray-600 italic">Select a club to see its graduation year distribution.</p>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="w-full h-96 p-4 bg-white rounded-lg shadow-md flex items-center justify-center">
        <p className="text-gray-600 italic">Loading graduation year distribution...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-96 p-4 bg-white rounded-lg shadow-md">
      <h2 className="text-xl font-bold text-black mb-4">Graduation Year Distribution</h2>
      {data.length > 0 ? (
        ...
        <p className="text-gray-600 italic">No graduation years recorded for this club.</p>
      )}
    </div>
  );
};

export default GraduationGraph;
